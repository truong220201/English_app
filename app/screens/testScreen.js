...
export default class testScreen extends React.Component {
    ...
    //speak
     async speak(say) {
        ...
        const options = {
          ...
          const r =  say.replace(/<\/?[^>]+(>|$)/g, "") ;
          //console.log(r)
          try{
            await Speech.speak(r,options,say)
          }
          catch{
    ...
    listenForItems(inum) {
        ...
        //[...Array(this.state.opt0.length)].map((o,n) => {
        //if(this.state.item[n]==this.state.itemK[inum-1]){
        console.log('ok dsfsdfs', this.state.opt1[0]);
    ...
    render() {
        ...
        //console.log(this.replace('item'));
        const item = '<p><span id="docs-internal-guid-c58cadc0-7fff-c312-af85-66a4c6bf63b1"><span style="font-size: 11pt; font-family: &quot;Times New Roman&quot;; background-color: transparent; font-variant-numeric: normal; font-variant-east-asian: normal; vertical-align: baseline; white-space: pre-wrap;">plays</span></span><br></p>';
                                ...
                                <ScrollView style={{ width: windowWidth, }} showsScrollIndicator={false}>
                                    <View style={styles.vw2}>
                                        ...
                                        <HTMLView
                                            value={questiona}
                                        />
                                        <TouchableOpacity onPress={this.speak(questiona)}>
                                        </TouchableOpacity>
                                                    ...
                                                    if (option == this.state.answ[this.state.itemQ - 1]) {
                                                        return (
                                                            <LinearGradient key={index} start={{ x: 0, y: 0.75 }} end={{ x: 1, y: 0.25 }} colors={['#6bdb91', '#6bdb91', '#6bdb91', '#b9f5dc']} style={{
                                                                ...
                                                                <TouchableOpacity onPress={this.speak(option)}>
                                                                </TouchableOpacity>
                                                            </LinearGradient>
                                                        );
                                                    } else {
